...
ssh_user: root
command: lsof -nP -L2 2>/dev/null|grep -E \'lib|bin\' |grep DEL|grep -v /SYSV00000000 | grep -v /var/lib/sss/mc | grep -v /etc/selinux/targeted/ | grep -v /elasticsearch/indices/ | cut -f 1 -d \' \' | sort -u
